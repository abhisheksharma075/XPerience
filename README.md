...
3. Apply the required database migrations in `supabase/migrations`.
4. Verify RLS policies for user-owned tables.
5. Add local environment variables to `.env.local`.
6. Test sign up, login, quest creation, quest completion, profile updates, and password recovery.
...
2. Whether the session is available when the query runs.
3. Whether the record contains the current user's ID.
4. Whether the corresponding RLS `INSERT` / `SELECT` / `UPDATE` policy exists.
5. Whether the user's profile row exists when a foreign key requires it.
...
**XPerience — Turn Your Life Into an RPG. 🎮⚔️**
